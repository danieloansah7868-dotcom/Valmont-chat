...
function serialize() {
  return {
    ...
    paymentLedger: paymentLedger.slice(-10000),
    paymentWebhookInbox: paymentWebhooks.slice(-5000),
  };
}

let persistPending = false;
let persistError = null;
let persistErrorAt = null;
...
function persistenceStatus() {
  if (persistError) return { ok: false, pending: persistPending, errorAt: persistErrorAt };
  return { ok: true, pending: !!persistPending };
}
...
function load() {
  try {
    ...
    if (raw.schemaVersion !== SCHEMA_VERSION) {
      throw new Error(`unsupported schema ${raw.schemaVersion}`);
    }
    ...
    for (const entry of raw.paymentLedger || []) paymentLedger.push(entry);
    for (const entry of raw.paymentWebhookInbox || raw.paymentWebhooks || []) paymentWebhooks.push(entry);
...
function accountView(user, sessionToken = null) {
  ...
  return {
    ...
    settings: getDeviceSettings(user.id),
  };
}
...
function verifyCode(phone, code) {
  ...
  pendingCodes.delete(phone);
  const continuation = crypto.randomBytes(24).toString('base64url');
  verifiedPhones.set(phone, {
    continuation,
...
function verificationRecord(phone) {
  ...
  return typeof record === 'number' ? { continuation: null, expiresAt: record } : record;
}

function isPhoneVerified(phone) {
  return Boolean(verificationRecord(phone));
}

function hasPhoneVerification(phone, continuation) {
  const record = verificationRecord(phone);
  return Boolean(record && continuation && record.continuation === continuation);
}

function consumePhoneVerification(phone, continuation) {
  const record = verificationRecord(phone);
  if (!record) return false;
  if (continuation && record.continuation !== continuation) return false;
  verifiedPhones.delete(phone);
  return true;
}
...
function searchUsers(query, exceptId) {
  ...
  const out = [];
  for (const user of users.values()) {
    if (user.deletedAt || (exceptId && user.id === exceptId)) continue;
...
const DEVICE_SETTINGS_DEFAULTS = Object.freeze({
  theme: 'light',
  lite: false,
  wallpaper: { id: 'default', dim: 40 },
  notifications: {
    desktop: false,
    previews: true,
    messageSounds: true,
...
function normalizeDeviceSettings(value = {}) {
  ...
  const wallpaperIn = incoming.wallpaper && typeof incoming.wallpaper === 'object' ? incoming.wallpaper : {};
  const notifyIn = incoming.notifications && typeof incoming.notifications === 'object' ? incoming.notifications : {};
  const custom = typeof wallpaperIn.custom === 'string' ? wallpaperIn.custom : '';
  const wallpaper = {
    id: String(wallpaperIn.id || 'default').slice(0, 40) || 'default',
    dim: Math.max(15, Math.min(75, Number(wallpaperIn.dim) || 40)),
  };
  if (custom.startsWith('data:image/') && custom.length <= 450000) wallpaper.custom = custom;
  return {
    theme: incoming.theme === 'dark' ? 'dark' : 'light',
    lite: incoming.lite === true,
    wallpaper,
    notifications: {
      desktop: notifyIn.desktop === true,
      previews: notifyIn.previews !== false,
      messageSounds: notifyIn.messageSounds !== false,
      messageTone: ['chime', 'soft', 'bright'].includes(notifyIn.messageTone) ? notifyIn.messageTone : 'chime',
      callSounds: notifyIn.callSounds !== false,
      ringtone: ['classic', 'gentle', 'pulse'].includes(notifyIn.ringtone) ? notifyIn.ringtone : 'classic',
      mediaVisibility: notifyIn.mediaVisibility === 'tap' ? 'tap' : 'show',
    },
    updatedAt: Number(incoming.updatedAt) || 0,
  };
}

function getDeviceSettings(userId) {
  const user = users.get(userId);
  if (!user || user.deletedAt) return null;
  return normalizeDeviceSettings(user.deviceSettings);
}

function updateDeviceSettings(userId, patch = {}) {
  const user = users.get(userId);
  if (!user || user.deletedAt) return null;
  user.deviceSettings = normalizeDeviceSettings({
    ...normalizeDeviceSettings(user.deviceSettings),
    ...patch,
    wallpaper: patch.wallpaper !== undefined
      ? { ...normalizeDeviceSettings(user.deviceSettings).wallpaper, ...patch.wallpaper }
      : normalizeDeviceSettings(user.deviceSettings).wallpaper,
    notifications: patch.notifications !== undefined
      ? { ...normalizeDeviceSettings(user.deviceSettings).notifications, ...patch.notifications }
      : normalizeDeviceSettings(user.deviceSettings).notifications,
    updatedAt: Date.now(),
  });
  save();
  return user.deviceSettings;
}

function setChatTransport(userId, chatId, transport) {
  const chat = chats.get(chatId);
  if (!chat?.members.has(userId) || chat.type !== 'dm') return null;
  if (!['cloud', 'sms'].includes(transport)) return null;
...
function routeInboundSms(fromPhone, body) {
  const sender = findUserByPhone(String(fromPhone || '').trim());
  const text = String(body || '').trim().slice(0, 1400);
  if (!sender || !text) return null;
  let best = null;
  let bestAt = -1;
  for (const chat of chats.values()) {
    if (chat.type !== 'dm' || !chat.members.has(sender.id)) continue;
    const lastSms = [...(messagesByChat.get(chat.id) || [])].reverse()
      .find(message => message.transport === 'sms');
    const stamp = lastSms?.timestamp || chat.createdAt || 0;
    if (stamp >= bestAt) {
      best = chat;
      bestAt = stamp;
    }
...
function getAllUsers(viewerId = null) {
  return [...users.values()]
    ...
    .map(user => publicUser(user, viewerId));
}
...
const SECRET_TTL_SECONDS = [0, 1, 5, 15, 60, 300, 3600, 86400];

function isValidSecretPublicKey(value) {
  return typeof value === 'string' && /^[A-Za-z0-9_-]{80,160}$/.test(value);
}

function isValidSecretEnvelope(encryption) {
  return Boolean(
    encryption
    && encryption.v === 1
    && encryption.alg === 'A256GCM'
    && /^[A-Za-z0-9_-]{16,32}$/.test(String(encryption.iv || ''))
    && /^[A-Za-z0-9_-]{16,20000}$/.test(String(encryption.ct || '')),
  );
}
...
function setDisappearing(chatId, actorId, seconds) {
  ...
  const allowed = chat?.type === 'secret' ? SECRET_TTL_SECONDS : [0, 86400, 604800, 7776000];
  if (!chat?.members.has(actorId) || !allowed.includes(value)) return null;
...
function findSecretChat(a, b) {
  for (const chat of chats.values()) {
    if (chat.type === 'secret' && chat.members.has(a) && chat.members.has(b)) return chat;
  }
  return null;
}

function createSecretChat(initiatorId, peerId, initiatorPublicKey) {
  if (!users.get(initiatorId) || !users.get(peerId) || initiatorId === peerId) {
    return { error: 'Contact not found' };
  }
  if (isBlockedBetween(initiatorId, peerId)) return { error: 'Messaging is unavailable for this contact' };
  const existing = findSecretChat(initiatorId, peerId);
  if (existing) return existing;
  if (!isValidSecretPublicKey(initiatorPublicKey)) return { error: 'A valid device key is required' };
  const id = uuidv4();
  const chat = {
    id,
    name: 'Secret chat',
    type: 'secret',
    about: '',
    members: new Set([initiatorId, peerId]),
    admins: new Set(),
    permissions: { editInfo: 'admins', sendMessages: 'members', addMembers: 'admins' },
    disappearingSeconds: 0,
...
function acceptSecretChat(chatId, userId, peerPublicKey) {
  const chat = chats.get(chatId);
  if (!chat || chat.type !== 'secret') return { error: 'Secret chat not found' };
  if (chat.secret?.peerId !== userId) return { error: 'Only the invited contact can accept' };
  if (chat.secret.state === 'ready') return chat;
  if (!isValidSecretPublicKey(peerPublicKey)) return { error: 'A valid device key is required' };
  chat.secret.peerPublicKey = peerPublicKey;
  chat.secret.state = 'ready';
  chat.secret.acceptedAt = Date.now();
  save();
  return chat;
}

function declineSecretChat(chatId, userId) {
  const chat = chats.get(chatId);
  if (!chat || chat.type !== 'secret' || !chat.members.has(userId)) return false;
  chats.delete(chatId);
  messagesByChat.delete(chatId);
  save();
  return true;
...
function secretStateView(chat, userId) {
  const secret = chat?.secret;
  if (!secret) return null;
  const initiator = userId === secret.initiatorId;
  return {
    state: secret.state,
    initiatorId: secret.initiatorId,
    peerId: secret.peerId,
    localPublicKey: initiator ? secret.initiatorPublicKey : secret.peerPublicKey,
    remotePublicKey: initiator ? secret.peerPublicKey : secret.initiatorPublicKey,
  };
}
...
function findOrCreateSaved(userId) {
  ...
  const chat = {
    ...
    name: 'Saved Messages',
    type: 'saved',
    about: 'Notes, files, and messages you send to yourself',
  ...
  chats.set(id, chat);
  if (!messagesByChat.has(id)) messagesByChat.set(id, []);
  save();
  return chat;
}

function listStarredMessages(userId, revealLocked = false) {
  const results = [];
  for (const chat of chats.values()) {
    ...
    for (const message of messagesByChat.get(chat.id) || []) {
      if (!message.starredBy?.has(userId) || !messageVisibleTo(message, userId)) continue;
      results.push({ chat: chatView(chat, userId), message: outMessage(message, userId) });
    }
  }
  return results.sort((a, b) => b.message.timestamp - a.message.timestamp).slice(0, 100);
}

function listCallHistory(userId, revealLocked = false) {
  const results = [];
  for (const chat of chats.values()) {
    if (!chat.members.has(userId) || (!revealLocked && isChatLocked(userId, chat.id))) continue;
    for (const message of messagesByChat.get(chat.id) || []) {
      if (message.type !== 'call' || !messageVisibleTo(message, userId)) continue;
      results.push({ chat: chatView(chat, userId), message: outMessage(message, userId) });
    }
  }
  return results.sort((a, b) => b.message.timestamp - a.message.timestamp).slice(0, 80);
}
...
function chatView(chat, userId) {
  ...
  if (chat.type === 'saved') {
    name = 'Saved Messages';
    peer = publicUser(users.get(userId), userId);
  } else if (chat.type === 'dm' || chat.type === 'secret') {
    const otherId = members.find(member => member !== userId) || userId;
    peer = publicUser(users.get(otherId), userId);
    name = chat.type === 'secret'
      ? (peer?.username ? `${peer.username}` : 'Secret chat')
  ...
  return {
    ...
    locked: !!state.locked,
    secret: secretStateView(chat, userId),
    transport: chat.type === 'dm' ? (state.transport === 'sms' ? 'sms' : 'cloud') : 'cloud',
  };
}
...
function messageVisibleTo(message, userId) {
  if (!message) return false;
  const state = getState(userId, message.chatId);
  if (message.timestamp <= (state.clearedAt || 0)) return false;
  if (message.deletedFor.has(userId)) return false;
  if (message.expiresAt && message.expiresAt <= Date.now()) return false;
  return true;
}

function storageStillReferenced(storageName, exceptAttachmentIds = new Set()) {
  if (!storageName) return false;
  for (const [id, attachment] of attachments) {
    if (exceptAttachmentIds.has(id)) continue;
    if (attachment.storageName === storageName) return true;
  }
  for (const user of users.values()) {
    if (user.profilePhoto?.storageName === storageName) return true;
  }
  for (const reel of reels.values()) if (reel.storageName === storageName) return true;
  for (const story of stories.values()) if (story.storageName === storageName) return true;
  for (const campaign of storyCampaigns.values()) if (campaign.storageName === storageName) return true;
  return false;
}

function releaseAttachmentIds(ids) {
  const names = [];
  const except = new Set(ids);
  for (const id of ids) {
    const attachment = attachments.get(id);
    if (!attachment) continue;
    attachments.delete(id);
    if (attachment.storageName && !storageStillReferenced(attachment.storageName, except)) {
      names.push(attachment.storageName);
    }
  }
...
function attachmentsForMessage(message) {
  if (!message?.file?.id) return [];
  const attachment = attachments.get(message.file.id);
  return attachment ? [attachment] : [];
}

function clearChat(userId, chatId) {
  const chat = chats.get(chatId);
  if (!chat?.members.has(userId)) return { storageNames: [] };
  const state = getState(userId, chatId, chatId);
  state.clearedAt = Date.now();
  const released = [];
  for (const message of messagesByChat.get(chatId) || []) {
    if (!message.file?.id) continue;
    const stillVisible = [...chat.members].some(memberId => memberId !== userId && messageVisibleTo(message, memberId));
    if (stillVisible) continue;
    released.push(...attachmentsForMessage(message).map(item => item.id));
    message.file = null;
  }
  const storageNames = releaseAttachmentIds(released);
  save();
  return { storageNames };
}
...
function sanitizeReactions(reactions = {}) {
  ...
  for (const [emoji, ids] of Object.entries(reactions || {})) {
    ...
    const unique = [...new Set(ids || [])].filter(id => users.has(id) && !users.get(id).deletedAt);
    if (unique.length) clean[emoji] = unique;
  }
  return clean;
}

function messagePreview(message) {
  if (!message || message.deleted) return 'Deleted message';
  if (message.text) return message.text;
  const mime = message.file?.mimeType || message.file?.mime || '';
...
function replySnapshot(chatId, messageId, viewerId) {
  ...
  if (!chat?.members.has(viewerId) || !message || message.del || !message || message.deleted || !messageVisibleTo(message, viewerId)) {
    return null;
  }
  ...
  return {
    ...
    senderName: sender?.deletedAt ? 'Deleted account' : (sender?.username || 'Unknown'),
    text,
    preview: text || messagePreview(message),
  };
}
...
function outMessage(message, viewerId) {
  ...
  return {
    ...
    reactions: message.deleted ? {} : sanitizeReactions(message.reactions),
    readBy: [...message.readBy],
    deliveredTo: [...message.deliveredTo],
    ...
    transport: message.transport === 'sms' ? 'sms' : 'cloud',
  };
}
...
function addMessage({ chatId, senderId, text, file, type, replyTo, call, clientId, encryption, forwarded, viewOnce = false, silent = false, mentions = [], transport = 'cloud' }) {
  const chat = chats.get(chatId);
  if (chat?.type === 'secret') {
    if (chat.secret?.state !== 'ready' || !isValidSecretEnvelope(encryption)) return null;
    text = '';
    file = null;
    call = null;
    mentions = [];
    ...
    viewOnce = false;
    transport = 'cloud';
    replyTo = replyTo?.id ? { id: String(replyTo.id) } : null;
  }
  if (transport === 'sms' && chat?.type !== 'dm') return null;
  ...
  const message = {
    ...
    forwarded: !!forwarded,
    silent: !!silent,
    mentions: [...new Set((Array.isArray(mentions) ? mentions : []).filter(id => users.has(id)))].slice(0, 64),
    viewOnce: !!viewOnce && !!file && /^(image|video)\//i.test(file.mimeType || file.mime || ''),
    openedBy: new Set([senderId]),
    encryption: encryption || null,
    transport: transport === 'sms' ? 'sms' : 'cloud',
  };
  const list = messagesByChat.get(chatId) || [];
  list.push(message);
...
function deleteMessage(chatId, messageId, userId, forEveryone) {
  const message = getRawMessage(chatId, messageId);
  const chat = chats.get(chatId);
  if (!message || !chat?.members.has(userId)) return null;
  let storageNames = [];
  if (forEveryone) {
    if (message.senderId !== userId && !canAdmin(chat, userId)) return null;
    const released = attachmentsForMessage(message).map(item => item.id);
    message.deleted = true;
    message.text = '';
    message.file = null;
    message.reactions = {};
    message.replyTo = null;
    storageNames = releaseAttachmentIds(released);
  } else {
    message.deletedFor.add(userId);
  }
  save();
  return Object.assign(message, { storageNames });
}

function toggleReaction(chatId, messageId, userId, emoji) {
  const chat = chats.get(chatId);
  const message = getRawMessage(chatId, messageId);
  if (!chat?.members.has(userId) || !message || message.deleted || !messageVisibleTo(message, userId)) return null;
  if (!ALLOWED_REACTIONS.has(String(emoji || ''))) return null;
  message.reactions ||= {};
...
function forwardMessage(sourceChatId, messageId, userId, targetChatIds = []) {
  ...
  if (!sourceChat?.members.has(userId) || sourceChat.advancedPrivacy || sourceChat.type === 'secret'
      || !original || original.deleted || original.viewOnce || original.encryption) return [];
  const sent = [];
...
function searchMessages(userId, query, revealLocked = false) {
  ...
  const results = [];
  for (const chat of chats.values()) {
    if (!chat.members.has(userId) || chat.type === 'secret' || (!revealLocked && isChatLocked(userId, chat.id))) continue;
    for (const message of messagesByChat.get(chat.id) || []) {
      if (message.deleted || message.deletedFor.has(userId) || (message.expiresAt && message.expiresAt <= Date.now())) continue;
      if (message.encryption || !message.text) continue;
  ...
  for (const [chatId, list] of messagesByChat) {
    ...
    for (const message of list) {
      if (message.expiresAt && message.expiresAt <= now) {
        ...
        const storageNames = releaseAttachmentIds(released);
        message.file = null;
        removed.push({ chatId, messageId: message.id, storageNames });
...
function getAttachment(id, viewerId, { allowViewOnce = false } = {}) {
  ...
  if (message && !messageVisibleTo(message, viewerId) && !allowViewOnce) return null;
...
function attachmentUsage(userId) {
  const owned = [...attachments.values()].filter(item => item.ownerId === userId);
  const pending = owned.filter(item => !item.messageId).length;
  const names = new Map();
  for (const item of owned) {
    if (!names.has(item.storageName)) names.set(item.storageName, item.size || 0);
  }
  return {
    bytes: [...names.values()].reduce((sum, size) => sum + size, 0),
    count: owned.length,
    pending,
  };
}
...
function discardAttachment(id, ownerId) {
  ...
  const storageNames = releaseAttachmentIds([id]);
  save();
  return { id, storageNames };
}

function pruneAbandonedAttachments(now = Date.now()) {
  const cutoff = now;
  const released = [];
  for (const [id, attachment] of attachments) {
    if (attachment.messageId) continue;
    if ((attachment.createdAt || 0) >= cutoff) continue;
    released.push(id);
  }
  const storageNames = releaseAttachmentIds(released);
  if (storageNames.length) save();
  return storageNames;
}
...
function finalizeViewOnceAttachment(chatId, messageId) {
  ...
  if (!recipients.length || !recipients.every(id => message.openedBy?.has(id))) return [];
  const released = attachmentsForMessage(message).map(item => item.id);
  const storageNames = releaseAttachmentIds(released);
  message.file = null;
  save();
...
function pruneConsumedViewOnce() {
  const names = [];
  for (const chat of chats.values()) {
    for (const message of messagesByChat.get(chat.id) || []) {
      if (message.viewOnce && message.file) names.push(...finalizeViewOnceAttachment(chat.id, message.id));
    }
  }
  return names;
}
...
function listPaymentLedger(campaignId, viewerId) {
  const campaign = storyCampaigns.get(campaignId);
  if (!campaign || campaign.advertiserId !== viewerId) return [];
  return paymentLedger.filter(entry => entry.campaignId === campaignId);
}
...
function confirmCampaignPayment(reference, { amount, currency, providerId } = {}) {
  ...
  if (['paid', 'partially_refunded', 'refunded'].includes(campaign.paymentStatus)) {
    if (providerId && campaign.paymentProviderId && String(providerId) !== String(campaign.paymentProviderId)) {
      return null;
    }
    return campaign;
  }
  ...
  campaign.capturedAmountMinor = campaign.expectedAmountMinor;
  campaign.refundedAmountMinor = campaign.refundedAmountMinor || 0;
  appendLedger({
    campaignId: campaign.id,
    kind: 'payment_captured',
    amountMinor: campaign.expectedAmountMinor,
    currency: 'GHS',
    providerId: campaign.paymentProviderId || null,
  });
...
function processValmontPayWebhook({ digest, event, status, reference, amount, currency, providerId } = {}) {
  const cleanDigest = String(digest || '');
  if (!cleanDigest) return { outcome: 'ignored' };
  const existing = paymentWebhooks.find(entry => entry.digest === cleanDigest);
  if (existing) return { duplicate: true, outcome: existing.outcome };
  let outcome = 'ignored';
  if (event === 'charge.success' && status === 'success') {
    const confirmed = confirmCampaignPayment(reference, { amount, currency, providerId });
    outcome = confirmed ? 'confirmed' : 'rejected';
  }
  paymentWebhooks.push({
    digest: cleanDigest, event, status, reference, amount, currency, providerId, outcome, at: Date.now(),
  });
  save();
  return { outcome };
}
...
function getPaymentWebhookInbox() {
  return paymentWebhooks.map(entry => ({ ...entry }));
}

function recordCampaignRefund(campaignId, { actorId, amountMinor, providerRefundId, reason } = {}) {
  const campaign = storyCampaigns.get(campaignId);
  const amount = Math.round(Number(amountMinor));
  if (!campaign || !users.has(actorId) || !Number.isFinite(amount) || amount <= 0) return null;
  const captured = Number(campaign.capturedAmountMinor || campaign.expectedAmountMinor || 0);
  const already = Number(campaign.refundedAmountMinor || 0);
  const prior = paymentLedger.find(entry => (
    (entry.kind === 'refund' || entry.kind === 'payment_refunded') && entry.providerId === providerRefundId
  ));
  if (prior) {
    return prior.amountMinor === amount ? prior : null;
  }
  if (already + amount > captured) return null;
  campaign.refundedAmountMinor = already + amount;
  campaign.paymentStatus = campaign.refundedAmountMinor >= captured ? 'refunded' : 'partially_refunded';
  appendLedger({
    campaignId: campaign.id,
    kind: 'payment_refunded',
    amountMinor: amount,
    currency: 'GHS',
    providerId: providerRefundId || null,
    actorId,
...
function listPaymentReconciliation() {
  const issues = [];
  for (const campaign of storyCampaigns.values()) {
    const captured = paymentLedger
      .filter(entry => entry.campaignId === campaign.id && entry.kind === 'payment_captured')
      .reduce((sum, entry) => sum + (entry.amountMinor || 0), 0);
    const refunded = paymentLedger
      .filter(entry => entry.campaignId === campaign.id && (entry.kind === 'refund' || entry.kind === 'payment_refunded'))
      .reduce((sum, entry) => sum + (entry.amountMinor || 0), 0);
    if (refunded > captured) issues.push({ campaignId: campaign.id, kind: 'over_refunded' });
  }
  return issues;
}
...
function protectedMediaUsage(userId, excludeStorageName = null) {
  const objects = new Map();
  const add = (name, size) => {
    if (!name || name === excludeStorageName || objects.has(name)) return;
    objects.set(name, Math.max(0, Number(size) || 0));
  };
  const user = users.get(userId);
  if (user?.profilePhoto?.storageName) add(user.profilePhoto.storageName, user.profilePhoto.size);
  for (const attachment of attachments.values()) {
    if (attachment.ownerId === userId) add(attachment.storageName, attachment.size);
  }
  for (const reel of reels.values()) {
    if (reel.ownerId === userId) add(reel.storageName, reel.size);
  }
  for (const story of stories.values()) {
    if (story.ownerId === userId) add(story.storageName, story.size);
  }
  for (const campaign of storyCampaigns.values()) {
    if (campaign.advertiserId === userId) add(campaign.storageName, campaign.size);
  }
  return {
    bytes: [...objects.values()].reduce((sum, size) => sum + size, 0),
...
function exportAccountData(userId) {
  ...
  return {
    ...
    chats: getUserChats(userId, true).map(chat => ({
      chat: { id: chat.id, type: chat.type, name: chat.name },
      messages: getMessages(chat.id, userId, 500).map(message => ({
        id: message.id,
        chatId: message.chatId,
        senderId: message.senderId,
        text: message.text,
    ...
    settings: getDeviceSettings(userId),
  };
}
...
function deleteAccount(userId, { requestedAt = Date.now() } = {}) {
  ...
  const storageNames = [];
  const consider = (name) => {
    if (name && !storageStillReferenced(name)) storageNames.push(name);
  };
  ...
  if (user.profilePhoto?.storageName) {
    const name = user.profilePhoto.storageName;
    user.profilePhoto = null;
    consider(name);
  }
  ...
  for (const [chatId, list] of messagesByChat) {
    for (const message of list) {
      if (message.senderId === userId) {
        ...
        message.file = null;
        message.reactions = {};
        storageNames.push(...releaseAttachmentIds(released));
      }
      if (message.reactions) {
        for (const [emoji, ids] of Object.entries(message.reactions)) {
          message.reactions[emoji] = (ids || []).filter(id => id !== userId);
    ...
    const chat = chats.get(chatId);
    if (chat?.type === 'group' && chat.members.has(userId)) {
      chat.admins?.delete(userId);
      if (chat.createdBy === userId || (chat.admins && !chat.admins.size)) {
        const successor = [...chat.members].find(id => id !== userId);
        if (successor) chat.admins.add(successor);
      }
    }
  }
  ...
  for (const [id, attachment] of [...attachments.entries()]) {
    ...
    const name = attachment.storageName;
    attachments.delete(id);
    consider(name);
  }

  for (const [id, story] of [...stories.entries()]) {
    if (story.ownerId !== userId) continue;
    const name = story.storageName;
    stories.delete(id);
    consider(name);
  }
  for (const [id, reel] of [...reels.entries()]) {
    if (reel.ownerId !== userId) continue;
    const name = reel.storageName;
    reels.delete(id);
    consider(name);
  }
  for (const campaign of storyCampaigns.values()) {
    if (campaign.advertiserId !== userId) continue;
    const name = campaign.storageName;
    campaign.storageName = null;
    campaign.text = '';
    campaign.billingEmail = '';
    campaign.deliveryStatus = 'stopped';
    campaign.stoppedAt = requestedAt;
  ...
  save();
  return { deletedAt: requestedAt, storageNames: [...new Set(storageNames)] };
}
...
module.exports = {
  ...
  normalizePhone, findUserByPhone, issueCode, verifyCode, isPhoneVerified, hasPhoneVerification, consumePhoneVerification,
  deleteAccount, exportAccountData, protectedMediaUsage,
  getDeviceSettings, updateDeviceSettings, setChatTransport, routeInboundSms,
  ...
  setDisappearing, setAdvancedPrivacy, findOrCreateDM, findOrCreateSaved,
  createSecretChat, acceptSecretChat, declineSecretChat, findSecretChat, isValidSecretEnvelope, SECRET_TTL_SECONDS,
  getUserChats, chatView, setChatFlag, clearChat, unreadCount,
  ...
  forwardMessage, searchMessages, listStarredMessages, listCallHistory, pruneExpiredMessages, replySnapshot,
  registerAttachment, getAttachment, validateAttachment, discardAttachment, openViewOnceMessage,
  attachmentUsage, pruneAbandonedAttachments, finalizeViewOnceAttachment, pruneConsumedViewOnce,
  ...
  listPaymentLedger, processValmontPayWebhook, getPaymentWebhookInbox, recordCampaignRefund,
