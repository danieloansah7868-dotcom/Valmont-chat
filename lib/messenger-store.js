...
function load() {
  try {
    ...
    for (const u of users.values()) {
      if (!u.handle) u.handle = suggestHandle(u.username || u.phone || u.id, u.id);
    }
...
function findUserByHandle(handle) {
  ...
  if (!h) return null;
  for (const u of users.values()) if (u.handle === h) return u;
  return null;
}
...
function searchUsers(query, exceptId) {
  ...
  for (const u of users.values()) {
    ...
    if (handle.startsWith(q) || handle.includes(q) || name.includes(q)) out.push(publicUser(u));
...
function publicUser(u, { self = false } = {}) {
  ...
  return {
    ...
    status: u.status,
    lastSeen: u.lastSeen,
    phone: self ? (u.phone || null) : undefined,
  };
}
 ...
 * New accounts need a unique @handle so friends can find them without a number.
 */
function upsertUserByPhone(phone, { username, handle, avatar, about } = {}) {
  let user = findUserByPhone(phone);
  if (!user) {
    const wanted = normalizeHandle(handle) || suggestHandle(username || phone);
    if (findUserByHandle(wanted)) return { error: 'That username is already taken' };
    const id = uuidv4();
    user = {
      id,
      phone,
      handle: wanted,
      username: (username && String(username).trim()) || wanted,
      avatar: avatar || null,
    ...
    if (handle) {
      ...
      if (!next) return { error: 'Username must be 3–20 characters, start with a letter, and use only letters, numbers or _' };
...
module.exports = {
  ...
  normalizeHandle, findUserByHandle, suggestHandle, searchUsers,
  // phone auth
