...
// Both maps are keyed by SHA-256(raw opaque capability), never by the raw
// browser cookie. Verification continuations are deliberately memory-only and
// single-use; a successful SMS challenge cannot be completed from another
// browser by racing on the phone number alone.
const verificationContinuations = new Map();
...
// Financial records are append-only. Campaign projections may change state, but
// captured payments and account-credit waivers are never edited or removed.
const paymentLedger = [];
// Signed provider deliveries are retained by a content digest so retries are
// idempotent and a crash cannot make a successfully acknowledged webhook vanish.
const paymentWebhookInbox = new Map();
const reports = [];
...
const MESSAGE_REACTION_SET = new Set(MESSAGE_REACTIONS);
...
function serialize() {
  return {
    ...
    paymentLedger: paymentLedger.slice(),
    paymentWebhookInbox: [...paymentWebhookInbox.entries()],
...
function writeSnapshot() {
  ...
  let descriptor = null;
  try {
    descriptor = fs.openSync(temp, 'w', 0o600);
    fs.writeFileSync(descriptor, JSON.stringify(serialize()), 'utf8');
    // A successful write is not durable until both the file contents and the
    // directory entry created by rename have reached the filesystem.
    fs.fsyncSync(descriptor);
    fs.closeSync(descriptor);
    descriptor = null;
    fs.renameSync(temp, DB_FILE);
    const directoryDescriptor = fs.openSync(DATA_DIR, 'r');
    try {
    ...
    lastSaveError = { message: error.message, at: Date.now() };
    throw error;
  }
}

/**
 * Coalesce ordinary mutations, while allowing acknowledgement and shutdown
 * boundaries to force a synchronous, fsync-backed commit with flush().
 */
function save() {
  dirty = true;
...
function flush() {
  ...
  if (!dirty) {
    if (lastSaveError) {
      const error = new Error(lastSaveError.message);
      error.code = 'ERR_STORE_PERSISTENCE';
      throw error;
    }
    return false;
  }
...
function persistenceStatus() {
  return lastSaveError
    ? { ok: false, pending: dirty, errorAt: lastSaveError.at }
    : { ok: true, pending: dirty };
}
...
function load() {
  try {
    ...
    if (raw.schemaVersion !== SCHEMA_VERSION) {
      const error = new Error(`Unsupported store schema ${raw.schemaVersion}; expected ${SCHEMA_VERSION}`);
      error.code = 'ERR_STORE_SCHEMA';
      throw error;
    }
    for (const saved of raw.users || []) {
      users.set(saved.id, {
        ...
        handle: normalizeHandle(saved.handle) || null,
    ...
    for (const entry of raw.paymentLedger || []) {
      if (!entry?.id || !entry.campaignId || !storyCampaigns.has(entry.campaignId)) continue;
      paymentLedger.push(Object.freeze({ ...entry }));
    }
    for (const [digest, entry] of raw.paymentWebhookInbox || []) {
      if (!/^[a-f0-9]{64}$/.test(String(digest)) || !entry || typeof entry !== 'object') continue;
      paymentWebhookInbox.set(digest, Object.freeze({ ...entry }));
    }
    for (const value of raw.reports || []) reports.push(value);
    for (const [key, value] of raw.inviteLinks || []) inviteLinks.set(key, value);
    return true;
  } catch (error) {
    error.code ||= 'ERR_STORE_LOAD';
    console.error('[store] load failed:', error.message);
    // Existing state must never be replaced by an apparently empty service.
    // Operators restore or migrate the snapshot before readiness can succeed.
    throw error;
  }
}
...
function findUserByPhone(phone) {
  ...
  for (const user of users.values()) if (!user.deletedAt && user.phone === phone) return user;
  return null;
}
...
function verifyCode(phone, code) {
  ...
  pendingCodes.delete(phone);
  const continuation = crypto.randomBytes(32).toString('base64url');
  const now = Date.now();
  for (const [digest, item] of verificationContinuations) {
    if (item.expiresAt <= now || item.phone === phone) verificationContinuations.delete(digest);
  }
  verificationContinuations.set(tokenDigest(continuation), {
    phone,
    createdAt: now,
...
function hasPhoneVerification(phone, continuation) {
  const digest = tokenDigest(continuation);
  const record = verificationContinuations.get(digest);
  if (!record || record.phone !== phone) return false;
  if (record.expiresAt <= Date.now()) {
    verificationContinuations.delete(digest);
    return false;
  }
  return true;
}

function consumePhoneVerification(phone, continuation) {
  if (!hasPhoneVerification(phone, continuation)) return false;
  verificationContinuations.delete(tokenDigest(continuation));
  return true;
}

function cancelCode(phone) {
  pendingCodes.delete(String(phone || ''));
}

function makeUser({ phone = null, username, handle = null, avatar = null, about = null, accountType = 'personal', businessProfile = null }) {
  const id = uuidv4();
...
function upsertUserByPhone(phone, { username, handle, avatar, about, accountType, businessProfile } = {}) {
  ...
  if (!user) user = makeUser({ phone, username, handle, avatar, about, accountType, businessProfile });
  else {
    if (username) user.username = String(username).trim();
    // A handle is claimed once at registration and is never reassigned here.
    if (handle && !user.handle) user.handle = normalizeHandle(handle);
...
// Handles are the public @identifier. They are lowercase, stable, and unique;
// the display name stays free-form and may be duplicated across accounts.
...
function validateHandle(value) {
  ...
  if (!handle) return { error: 'Choose a username' };
...
function findUserByHandle(value) {
  const handle = normalizeHandle(value);
  if (!handle) return null;
  for (const user of users.values()) if (!user.deletedAt && user.handle === handle) return user;
  return null;
}

// Derives a unique handle from a display name for callers that do not supply
// one (older clients, scripted/test registrations). Never returns a clash.
function deriveHandle(seed) {
  let base = normalizeHandle(seed).replace(/[^a-z0-9_]/g, '').slice(0, 20);
  while (base.length < 3) base += '0';
  if (!findUserByHandle(base)) return base;
  for (let n = 1; n < 10000; n++) {
    const suffix = String(n);
    const candidate = base.slice(0, 20 - suffix.length) + suffix;
    if (!findUserByHandle(candidate)) return candidate;
  }
  return `u${crypto.randomBytes(6).toString('hex')}`.slice(0, 20);
}

function handleAvailable(value, forUserId = null) {
  const { handle, error } = validateHandle(value);
  if (error) return { available: false, error };
  const taken = findUserByHandle(handle);
  if (taken && taken.id !== forUserId) return { available: false, handle, error: 'That username is already taken' };
...
function updateProfile(userId, { username, handle, avatar, about }) {
  ...
  if (handle !== undefined && handle !== null && normalizeHandle(handle) !== user.handle) {
    const check = handleAvailable(handle, userId);
    if (!check.available) return { error: check.error };
    user.handle = check.handle;
  }
  if (avatar !== undefined) user.avatar = avatar;
...
function exportAccountData(userId) {
  ...
  const chatExports = [...chats.values()]
    ...
    .map(chat => {
      ...
      return { chat: chatView(chat, userId), messages };
    });
  const ownedCampaigns = [...storyCampaigns.values()]
    .filter(campaign => campaign.advertiserId === userId);
  const activeSessions = listSessions(userId).map(session => ({
    id: session.id,
    label: session.label,
  ...
  return {
    ...
    stories: [...stories.values()]
      ...
      .map(story => storyView(story, userId))
      .filter(Boolean),
    reels: [...reels.values()]
      .filter(reel => reel.ownerId === userId)
      .map(reel => reelView(reel, userId))
      .filter(Boolean),
    storyCampaigns: ownedCampaigns.map(campaign => ({
      campaign: campaignView(campaign, userId),
      paymentLedger: paymentLedger
        .filter(entry => entry.campaignId === campaign.id)
        .map(entry => ({ ...entry })),
    })),
    ...
    callRatings: [...callRatings.values()]
      .filter(rating => rating.userId === userId)
      .map(rating => ({ ...rating })),
  };
}

function deleteAccount(userId, { requestedAt = Date.now() } = {}) {
  const user = users.get(userId);
  const timestamp = Number(requestedAt);
  if (!user || user.deletedAt || !Number.isSafeInteger(timestamp) || timestamp <= 0) return null;
  const previousPhone = user.phone;
  const mediaCandidates = new Set();
  const changedMessages = [];
  if (user.profilePhoto?.storageName) mediaCandidates.add(user.profilePhoto.storageName);

  // Recipient copies retain a tombstone, but the deleting account's message
  // bodies, replies, reactions, and attachment references are erased.
  for (const list of messagesByChat.values()) {
    for (const message of list) {
      ...
      if (message.senderId === userId && !message.deleted) {
        ...
        message.text = '';
        for (const storageName of detachMessageAttachment(message)) mediaCandidates.add(storageName);
        message.reactions = {};
      ...
      message.readBy?.delete(userId);
      message.deliveredTo?.delete(userId);
      message.deletedFor?.delete(userId);
      message.starredBy?.delete(userId);
      message.openedBy?.delete(userId);
      if (message.replyTo?.senderId === userId) {
        message.replyTo = {
          ...message.replyTo,
      ...
      for (const emoji of Object.keys(message.reactions || {})) {
        const previousLength = message.reactions[emoji].length;
        message.reactions[emoji] = message.reactions[emoji].filter(id => id !== userId);
        if (message.reactions[emoji].length !== previousLength) changed = true;
        if (!message.reactions[emoji].length) delete message.reactions[emoji];
      }
      if (changed) changedMessages.push({ chatId: message.chatId, messageId: message.id });
    }
  }
  for (const [id, attachment] of [...attachments.entries()]) {
    if (attachment.ownerId !== userId) continue;
    for (const storageName of removeAttachmentMetadata(id)) mediaCandidates.add(storageName);
  }
  for (const [id, story] of [...stories.entries()]) {
    if (story.ownerId === userId) {
      if (story.storageName) mediaCandidates.add(story.storageName);
      stories.delete(id);
      continue;
    }
    story.viewedBy.delete(userId);
    story.reactions.delete(userId);
  }
  for (const [id, reel] of [...reels.entries()]) {
    if (reel.ownerId === userId) {
      if (reel.storageName) mediaCandidates.add(reel.storageName);
      reels.delete(id);
    } else reel.likedBy.delete(userId);
  }
  for (const campaign of storyCampaigns.values()) {
    if (campaign.impressionUsers.delete(userId)) campaign.impressionCount = Math.max(0, campaign.impressionCount - 1);
    if (campaign.clickUsers.delete(userId)) campaign.clickCount = Math.max(0, campaign.clickCount - 1);
    if (campaign.advertiserId !== userId) continue;
    if (campaign.storageName) mediaCandidates.add(campaign.storageName);
    campaign.storageName = null;
    campaign.size = 0;
    campaign.mime = null;
    campaign.type = 'text';
    campaign.text = '';
    campaign.background = 'midnight';
    campaign.destinationUrl = '';
    campaign.billingEmail = '';
    campaign.sourceStoryId = null;
    campaign.checkoutUrl = null;
    if (['pending', 'active', 'paused'].includes(campaign.deliveryStatus)) {
      campaign.deliveryStatus = 'stopped';
      campaign.stoppedBy = userId;
      campaign.stoppedAt = timestamp;
      campaign.stopNote = 'Advertiser account deleted';
    }
  }
  ...
  for (const [key, rating] of [...callRatings.entries()]) {
    if (rating.userId === userId) callRatings.delete(key);
  }
  for (const key of [...chatState.keys()]) {
    if (key.startsWith(`${userId}:`)) chatState.delete(key);
  }
  if (previousPhone) pendingCodes.delete(previousPhone);
  for (const [digest, continuation] of [...verificationContinuations.entries()]) {
    if (continuation.phone === previousPhone) verificationContinuations.delete(digest);
  }
  for (const invite of inviteLinks.values()) {
    if (invite.createdBy === userId && !invite.revokedAt) invite.revokedAt = timestamp;
  }
  for (const other of users.values()) other.blocked?.delete(userId);
  for (const chat of chats.values()) {
    chat.admins.delete(userId);
    if (chat.type === 'group' && chat.members.size > 1 && chat.admins.size === 0) {
      const replacement = [...chat.members].find(id => id !== userId && !users.get(id)?.deletedAt);
      if (replacement) chat.admins.add(replacement);
    }
  }
  ...
  user.blocked = new Set();
  user.privacy = {
    ...DEFAULT_PRIVACY,
    lastSeen: 'nobody', profilePhoto: 'nobody', about: 'nobody', online: 'same-as-last-seen',
  };
  user.pinHash = null;
  user.pinSalt = null;
  user.chatLock = normalizeChatLock();
  user.deletedAt = timestamp;

  // Shared bytes survive if any remaining profile, attachment, reel, story, or
  // retained campaign still references the same protected object.
  const referenced = storageName => {
    const name = path.basename(String(storageName || ''));
    if (!name) return false;
    if ([...users.values()].some(item => path.basename(String(item.profilePhoto?.storageName || '')) === name)) return true;
    if ([...attachments.values()].some(item => path.basename(String(item.storageName || '')) === name)) return true;
    if ([...reels.values()].some(item => path.basename(String(item.storageName || '')) === name)) return true;
    if ([...stories.values()].some(item => path.basename(String(item.storageName || '')) === name)) return true;
    return [...storyCampaigns.values()].some(item => path.basename(String(item.storageName || '')) === name);
  };
  save();
  return {
    deletedAt: timestamp,
    changedMessages,
    storageNames: [...mediaCandidates]
      .map(name => path.basename(String(name)))
      .filter(name => name && !referenced(name)),
  };
}
...
function getAllUsers(viewerId = null) {
  return [...users.values()].filter(user => !user.deletedAt).map(user => publicUser(user, viewerId));
}

function getOnlineUsers(viewerId = null) {
  return [...users.values()].filter(user => !user.deletedAt && user.status === 'online').map(user => publicUser(user, viewerId));
}
...
function clearChat(userId, chatId) {
  if (!chats.get(chatId)?.members.has(userId)) return null;
  const state = getState(userId, chatId);
  state.clearedAt = Date.now();
  const storageNames = [];
  for (const message of messagesByChat.get(chatId) || []) {
    if (message.file?.id && !messageHasVisibleMember(message)) {
      storageNames.push(...detachMessageAttachment(message));
    }
  }
  save();
  return { ok: true, storageNames };
}
...
function safeMessageReactions(reactions) {
  const safe = {};
  for (const emoji of MESSAGE_REACTIONS) {
    const userIds = Array.isArray(reactions?.[emoji])
      ? [...new Set(reactions[emoji].filter(id => users.has(id)))] : [];
    if (userIds.length) safe[emoji] = userIds;
  }
  return safe;
}
...
function outMessage(message, viewerId) {
  ...
  const visibleFile = message.deleted || openedByViewer ? null : (message.file ? {
    ...message.file,
    // View Once bytes are never available through the ordinary media route,
    // including to the sender. A recipient receives a one-use grant only after
    // atomically opening the message.
    url: viewOnce ? null : message.file.url,
  } : null);
  return {
    id: message.id,
    ...
    reactions: message.deleted ? {} : safeMessageReactions(message.reactions),
    readBy: [...message.readBy],
    deliveredTo: [...message.deliveredTo],
...
function messageVisibleTo(message, userId, now = Date.now()) {
  if (!message || message.deleted || message.deletedFor?.has(userId)
      || (message.expiresAt && message.expiresAt <= now)) return false;
  return message.timestamp > (getState(userId, message.chatId).clearedAt || 0);
}

function replySnapshot(chatId, messageId, viewerId) {
  const chat = chats.get(chatId);
  const message = getRawMessage(chatId, String(messageId || ''));
  if (!chat?.members.has(viewerId) || !messageVisibleTo(message, viewerId)) return null;
  const sender = users.get(message.senderId);
  const text = String(message.text || '').slice(0, 500);
  let preview = text;
  if (!preview && message.file) preview = message.viewOnce
    ? 'View Once media' : String(message.file.name || 'Attachment').slice(0, 255);
  if (!preview && message.type === 'call') preview = 'Call';
  if (!preview) preview = 'Message';
  return {
    id: message.id,
    senderId: message.senderId,
    senderName: String(sender?.username || 'Unknown').slice(0, 80),
    text,
    preview,
  };
}
...
function deleteMessage(chatId, messageId, userId, forEveryone) {
  ...
  if (forEveryone) {
    ...
    message.deletedFor.add(userId);
    if (message.file?.id && !messageHasVisibleMember(message)) {
      storageNames.push(...detachMessageAttachment(message));
    }
  }
  save();
  return { message, storageNames };
}

function toggleReaction(chatId, messageId, userId, emoji) {
  const message = getRawMessage(chatId, messageId);
  if (!chats.get(chatId)?.members.has(userId) || !messageVisibleTo(message, userId)
      || !MESSAGE_REACTION_SET.has(emoji)) return null;
  message.reactions = safeMessageReactions(message.reactions);
    ...
    for (const message of list) {
      if (message.expiresAt && message.expiresAt <= now) {
        removed.push({
          ...
          storageNames: detachMessageAttachment(message),
        });
...
function removeAttachmentMetadata(id) {
  const attachment = attachments.get(id);
  if (!attachment) return [];
  attachments.delete(id);
  const stillReferenced = [...attachments.values()]
    .some(item => item.storageName === attachment.storageName);
  return stillReferenced || !attachment.storageName ? [] : [attachment.storageName];
}

function detachMessageAttachment(message) {
  const attachmentId = message?.file?.id;
  if (!attachmentId) return [];
  message.file = null;
  return removeAttachmentMetadata(attachmentId);
}

function messageHasVisibleMember(message, now = Date.now()) {
  const chat = chats.get(message?.chatId);
  if (!chat || message.deleted || (message.expiresAt && message.expiresAt <= now)) return false;
  return [...chat.members].some(memberId => messageVisibleTo(message, memberId, now));
}
...
function getAttachment(id, viewerId, { allowViewOnce = false } = {}) {
  ...
  // Before a file is posted, only its uploader may preview it.
  if (!attachment.messageId) return attachment.ownerId === viewerId ? attachment : null;

  // Claimed bytes are authorized by a currently visible message, not merely by
  // current chat membership or possession of an old attachment identifier.
  const message = getRawMessage(attachment.chatId, attachment.messageId);
  if (!messageVisibleTo(message, viewerId)
      || message.file?.id !== attachment.id) return null;
  if (message.viewOnce) {
    if (!allowViewOnce || message.senderId === viewerId
        || !message.openedBy?.has(viewerId)) return null;
  }
  return attachment;
}
...
function openViewOnceMessage(chatId, messageId, viewerId) {
  ...
  if (!chat?.members.has(viewerId) || !message?.viewOnce || message.deleted
      ...
      || !messageVisibleTo(message, viewerId)) return null;
...
function finalizeViewOnceAttachment(chatId, messageId) {
  const chat = chats.get(chatId);
  const message = getRawMessage(chatId, messageId);
  if (!chat || !message?.viewOnce || !message.file?.id) return [];
  const recipients = [...chat.members].filter(id => id !== message.senderId);
  if (recipients.some(id => !message.openedBy?.has(id))) return [];
  const storageNames = detachMessageAttachment(message);
  save();
  return storageNames;
}

function pruneConsumedViewOnceAttachments() {
  const storageNames = [];
  let changed = false;
  for (const [chatId, messages] of messagesByChat) {
    for (const message of messages) {
      if (!message.viewOnce || !message.file?.id) continue;
      const chat = chats.get(chatId);
      const recipients = [...(chat?.members || [])].filter(id => id !== message.senderId);
      if (recipients.some(id => !message.openedBy?.has(id))) continue;
      storageNames.push(...detachMessageAttachment(message));
      changed = true;
    }
  }
...
function discardAttachment(id, ownerId) {
  const attachment = attachments.get(id);
  if (!attachment || attachment.ownerId !== ownerId || attachment.messageId) return null;
  const storageNames = removeAttachmentMetadata(id);
  save();
  return storageNames;
}

function pruneAbandonedAttachments(now = Date.now(), maxAgeMs = 60 * 60 * 1000) {
  const storageNames = [];
  let changed = false;
  for (const attachment of [...attachments.values()]) {
    if (!attachment.messageId && now - attachment.createdAt >= maxAgeMs) {
      storageNames.push(...removeAttachmentMetadata(attachment.id));
      changed = true;
    }
  }
  if (changed) save();
...
function attachmentUsage(ownerId) {
  let bytes = 0;
  let count = 0;
  let pending = 0;
  const countedStorage = new Set();
  for (const attachment of attachments.values()) {
    if (attachment.ownerId !== ownerId) continue;
    if (!countedStorage.has(attachment.storageName)) {
      bytes += Math.max(0, Number(attachment.size) || 0);
      countedStorage.add(attachment.storageName);
    }
    count += 1;
    if (!attachment.messageId) pending += 1;
  }
  return { bytes, count, pending };
}

function protectedMediaUsage(ownerId, ignoredStorageName = null) {
  const sizes = new Map();
  const include = (storageName, size) => {
    const name = storageName ? path.basename(String(storageName)) : '';
    if (!name || name === ignoredStorageName) return;
    sizes.set(name, Math.max(sizes.get(name) || 0, Math.max(0, Number(size) || 0)));
  };
  const user = users.get(ownerId);
  if (user?.profilePhoto) include(user.profilePhoto.storageName, user.profilePhoto.size);
  for (const attachment of attachments.values()) {
    if (attachment.ownerId === ownerId) include(attachment.storageName, attachment.size);
  }
  for (const reel of reels.values()) {
    if (reel.ownerId === ownerId) include(reel.storageName, reel.size);
  }
  for (const story of stories.values()) {
    if (story.ownerId === ownerId) include(story.storageName, story.size);
  }
  for (const campaign of storyCampaigns.values()) {
    if (campaign.advertiserId === ownerId) include(campaign.storageName, campaign.size);
  }
  return { bytes: [...sizes.values()].reduce((total, size) => total + size, 0), objects: sizes.size };
}
...
function campaignView(campaign, viewerId, { admin = false } = {}) {
  ...
  return {
    ...
    refundedAmountMinor: ownerView ? campaign.refundedAmountMinor || 0 : undefined,
...
function appendPaymentLedgerEntry({
  campaignId, reference, kind, amountMinor, currency = 'GHS', providerId = null,
  actorId = null, source, idempotencyKey, reason = null, at = Date.now(),
}) {
  const existing = paymentLedger.find(entry => entry.idempotencyKey === idempotencyKey);
  ...
  const entry = Object.freeze({
    ...
    providerId: providerId ? String(providerId).slice(0, 200) : null,
    actorId: actorId || null,
    source,
    idempotencyKey,
    reason: reason ? [...String(reason)].slice(0, 300).join('') : null,
    createdAt: at,
  });
...
function confirmCampaignPayment(reference, { amount, currency, providerId, source = 'provider_verification' } = {}) {
  const cleanReference = String(reference || '').trim().slice(0, 120);
  const cleanCurrency = String(currency || '').toUpperCase();
  const cleanProviderId = providerId ? String(providerId).trim().slice(0, 200) : null;
  const cleanAmount = Number(amount);
  const campaign = findCampaignByPaymentReference(cleanReference);
  if (!campaign || cleanCurrency !== 'GHS' || !Number.isSafeInteger(cleanAmount)
      || cleanAmount !== campaign.expectedAmountMinor) return null;

  const existingCapture = paymentLedger.find(entry => (
    entry.kind === 'payment_captured' && entry.campaignId === campaign.id
  ));
  if (existingCapture) {
    const sameProvider = !existingCapture.providerId || !cleanProviderId
      || existingCapture.providerId === cleanProviderId;
    return existingCapture.reference === cleanReference
      && existingCapture.amountMinor === cleanAmount
      && existingCapture.currency === cleanCurrency
      && sameProvider ? campaign : null;
  }
  if (cleanProviderId && paymentLedger.some(entry => (
    entry.kind === 'payment_captured' && entry.providerId === cleanProviderId
  ))) return null;
  ...
  appendPaymentLedgerEntry({
    ...
    source,
    idempotencyKey: cleanProviderId
      ? `valmontpay:capture:${cleanProviderId}`
      : `valmontpay:capture-reference:${cleanReference}`,
  });
  campaign.paymentStatus = 'paid';
  campaign.paymentProviderId = cleanProviderId || campaign.paymentProviderId;
...
function processValmontPayWebhook({
  digest, event, status, reference, amount, currency, providerId,
} = {}) {
  const key = String(digest || '').toLowerCase();
  if (!/^[a-f0-9]{64}$/.test(key)) return null;
  const existing = paymentWebhookInbox.get(key);
  if (existing) {
    return {
      duplicate: true,
      outcome: existing.outcome,
      campaign: existing.campaignId ? storyCampaigns.get(existing.campaignId) || null : null,
    };
  }
  ...
  if (event === 'charge.success' && status === 'success') {
    campaign = confirmCampaignPayment(reference, {
      amount,
      currency,
      providerId,
      source: 'signed_webhook',
    });
    outcome = campaign ? 'confirmed' : 'rejected';
  }
  const record = Object.freeze({
    ...
    providerId: providerId ? String(providerId).slice(0, 200) : null,
    amountMinor: Number.isSafeInteger(Number(amount)) ? Number(amount) : null,
    currency: String(currency || '').toUpperCase().slice(0, 8),
    outcome,
    campaignId: campaign?.id || null,
    receivedAt: Date.now(),
  });
  paymentWebhookInbox.set(key, record);
  save();
  return { duplicate: false, outcome, campaign };
}

function listPaymentLedger(campaignId, actorId, { admin = false } = {}) {
  const campaign = storyCampaigns.get(campaignId);
  if (!campaign || (!admin && campaign.advertiserId !== actorId)) return [];
  return paymentLedger.filter(entry => entry.campaignId === campaignId).map(entry => ({ ...entry }));
}

function getPaymentWebhookInbox() {
  return [...paymentWebhookInbox.values()].map(entry => ({ ...entry }));
}

/**
 * Record a refund that an advertising administrator has already completed in
 * ValmontPay. The provider contract currently available to Vchat has no refund
 * operation, so this is deliberately an accounting record—not a claim that
 * calling this function moved money. Provider refund IDs are globally unique
 * and make retries idempotent.
 */
function recordCampaignRefund(campaignId, {
  actorId, amountMinor, providerRefundId, reason, occurredAt = Date.now(),
} = {}) {
  const campaign = storyCampaigns.get(campaignId);
  const amount = Number(amountMinor);
  const refundId = String(providerRefundId || '').trim().slice(0, 200);
  const cleanReason = [...String(reason || '').trim()].slice(0, 300).join('');
  const timestamp = Number(occurredAt);
  if (!campaign || !users.has(actorId) || campaign.paymentStatus === 'waived'
      || !Number.isSafeInteger(amount) || amount <= 0 || !refundId || !cleanReason
      || !Number.isSafeInteger(timestamp) || timestamp <= 0) return null;

  const existing = paymentLedger.find(entry => (
    entry.kind === 'payment_refunded' && entry.providerId === refundId
  ));
  if (existing) {
    return existing.campaignId === campaign.id && existing.amountMinor === amount
      && existing.currency === 'GHS' ? campaign : null;
  }

  const capture = paymentLedger.find(entry => (
    entry.kind === 'payment_captured' && entry.campaignId === campaign.id
  ));
  if (!capture || capture.currency !== 'GHS') return null;
  const refunded = paymentLedger
    .filter(entry => entry.kind === 'payment_refunded' && entry.campaignId === campaign.id)
    .reduce((total, entry) => total + entry.amountMinor, 0);
  if (refunded + amount > capture.amountMinor) return null;
  ...
  appendPaymentLedgerEntry({
    ...
    providerId: refundId,
    actorId,
    source: 'valmontpay_admin_confirmation',
    reason: cleanReason,
    at: timestamp,
    idempotencyKey: `valmontpay:refund:${refundId}`,
  });
  campaign.refundedAmountMinor = refunded + amount;
  campaign.paymentStatus = campaign.refundedAmountMinor === capture.amountMinor
    ? 'refunded' : 'partially_refunded';
  campaign.lastRefundAt = timestamp;
  if (campaign.paymentStatus === 'refunded'
      && ['pending', 'active', 'paused'].includes(campaign.deliveryStatus)) {
    campaign.deliveryStatus = 'stopped';
    campaign.stoppedBy = actorId;
    campaign.stoppedAt = timestamp;
    campaign.stopNote = cleanReason;
  }
  save();
  return campaign;
}

/** Return deterministic accounting exceptions for operator reconciliation. */
function listPaymentReconciliation({ now = Date.now(), pendingAfterMs = 24 * 60 * 60 * 1000 } = {}) {
  const issues = [];
  for (const campaign of storyCampaigns.values()) {
    const entries = paymentLedger.filter(entry => entry.campaignId === campaign.id);
    const captures = entries.filter(entry => entry.kind === 'payment_captured');
    const refunds = entries.filter(entry => entry.kind === 'payment_refunded');
    const capturedMinor = captures.reduce((total, entry) => total + entry.amountMinor, 0);
    const refundedMinor = refunds.reduce((total, entry) => total + entry.amountMinor, 0);
    const push = (code, detail) => issues.push({
      campaignId: campaign.id,
      advertiserId: campaign.advertiserId,
      reference: campaign.paymentReference,
      code,
      detail,
      expectedAmountMinor: campaign.expectedAmountMinor,
      capturedMinor,
      refundedMinor,
      currency: 'GHS',
    });

    if (captures.length > 1) push('multiple_captures', 'More than one capture exists for the campaign');
    if (captures.length === 1 && capturedMinor !== campaign.expectedAmountMinor) {
      push('capture_amount_mismatch', 'The captured total differs from the reserved campaign budget');
    }
    if (['paid', 'partially_refunded', 'refunded'].includes(campaign.paymentStatus) && captures.length === 0) {
      push('missing_capture', 'Campaign state says money was received but no capture ledger entry exists');
    }
    if (captures.length > 0 && !['paid', 'partially_refunded', 'refunded'].includes(campaign.paymentStatus)) {
      push('capture_state_mismatch', 'A capture exists but campaign payment state does not reflect it');
    }
    if (refundedMinor > capturedMinor) push('refund_exceeds_capture', 'Refund total exceeds captured funds');
    if (campaign.paymentStatus === 'refunded' && refundedMinor !== capturedMinor) {
      push('refund_state_mismatch', 'Campaign is marked refunded but ledger totals do not balance');
    }
    if (campaign.paymentStatus === 'partially_refunded'
        && (refundedMinor <= 0 || refundedMinor >= capturedMinor)) {
      push('partial_refund_state_mismatch', 'Partial-refund state does not match ledger totals');
    }
    if (campaign.paymentStatus === 'pending' && campaign.createdAt + pendingAfterMs <= now) {
      push('stale_pending_payment', 'Initialized checkout has no capture after the reconciliation threshold');
    }
  }
  return issues.sort((a, b) => a.campaignId.localeCompare(b.campaignId) || a.code.localeCompare(b.code));
}
...
function reviewStoryCampaign(id, { approved, note, waivePayment = false, reviewerId } = {}) {
  ...
  if (approved && waivePayment && campaign.paymentStatus !== 'paid') {
    appendPaymentLedgerEntry({
      campaignId: campaign.id,
      reference: campaign.paymentReference || `credit-${campaign.id}`,
      kind: 'account_credit_waiver',
      amountMinor: campaign.expectedAmountMinor,
      actorId: reviewerId,
      source: 'admin_review',
      reason: cleanNote,
      idempotencyKey: `account-credit-waiver:${campaign.id}`,
    });
...
module.exports = {
  ...
  MAX_CHAT_LOCK_CREDENTIALS, MESSAGE_REACTIONS,
  // users and auth
  ...
  normalizeHandle, validateHandle, findUserByHandle, handleAvailable, deriveHandle,
  normalizePhone, findUserByPhone, issueCode, verifyCode, hasPhoneVerification, consumePhoneVerification, cancelCode,
  ...
  blockUser, isBlockedBetween, isKnownContact, exportAccountData, deleteAccount, reportUser,
  // chats and groups
  ...
  // messages
  addMessage, findByClientId, getMessages, getRawMessage, replySnapshot, outMessage, markRead, markDelivered,
  ...
  // media and reels
  registerAttachment, attachmentUsage, protectedMediaUsage, getAttachment, validateAttachment, discardAttachment, openViewOnceMessage,
  finalizeViewOnceAttachment, pruneConsumedViewOnceAttachments, pruneAbandonedAttachments,
  ...
  confirmCampaignPayment, processValmontPayWebhook, listPaymentLedger, getPaymentWebhookInbox,
  recordCampaignRefund, listPaymentReconciliation, reviewStoryCampaign, controlStoryCampaign, listStoryCampaigns, listStoryCampaignsForReview,
