...
function serialize() {
  return {
    ...
    paymentWebhooks: paymentWebhooks.slice(-5000),
  };
}
...
function load() {
  try {
    ...
    for (const entry of raw.paymentWebhooks || []) paymentWebhooks.push(entry);
...
function recordCampaignRefund(campaignId, { actorId, amountMinor, providerRefundId, reason } = {}) {
  ...
  if (!campaign || campaign.advertiserId !== actorId || !Number.isFinite(amount) || amount <= 0) return null;
  ...
  const prior = paymentLedger.find(entry => entry.kind === 'refund' && entry.providerId === providerRefundId);
  if (prior) {
...
function listPaymentReconciliation() {
  ...
  for (const campaign of storyCampaigns.values()) {
    ...
    const refunded = paymentLedger
      .filter(entry => entry.campaignId === campaign.id && entry.kind === 'refund')
      .reduce((sum, entry) => sum + (entry.amountMinor || 0), 0);
...
function exportAccountData(userId) {
  ...
  return {
    ...
    chats: getUserChats(userId, true).map(chat => chat.id),
  };
}
