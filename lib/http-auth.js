...
const VERIFY_COOKIE_NAME = 'vchat_verify';
...
function verifyCookieAttributes(maxAgeSeconds, token = '') {
  return [
    `${VERIFY_COOKIE_NAME}=${encodeURIComponent(token)}`,
    'Path=/',
    'HttpOnly',
    'SameSite=Strict',
    `Max-Age=${Math.max(0, Math.floor(maxAgeSeconds))}`,
...
function setVerifyCookie(res, continuation) {
  res.append('Set-Cookie', verifyCookieAttributes(10 * 60, continuation).join('; '));
}

function clearVerifyCookie(res) {
  res.append('Set-Cookie', verifyCookieAttributes(0).join('; '));
}

function verifyContinuationFromRequest(req) {
  const cookies = parseCookies(req.headers?.cookie);
  return cookies[VERIFY_COOKIE_NAME] || '';
}
