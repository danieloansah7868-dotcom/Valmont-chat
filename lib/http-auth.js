...
const VERIFICATION_COOKIE_NAME = COOKIE_SECURE ? '__Host-vchat_verify' : 'vchat_verify';
...
function verificationTokenFromRequest(req) {
  return parseCookies(req.headers?.cookie)[VERIFICATION_COOKIE_NAME] || '';
}
...
function verificationCookieAttributes(maxAgeSeconds) {
  const attrs = [
  ...
  if (COOKIE_SECURE || VERIFICATION_COOKIE_NAME.startsWith('__Host-')) attrs.push('Secure');
  return attrs;
}

function setVerificationCookie(res, continuation) {
  const attrs = verificationCookieAttributes(10 * 60);
  attrs[0] += encodeURIComponent(continuation);
  res.append('Set-Cookie', attrs.join('; '));
}

function clearVerificationCookie(res) {
  res.append('Set-Cookie', verificationCookieAttributes(0).join('; '));
}
