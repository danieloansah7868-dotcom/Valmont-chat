...
function attach(httpServer, app) {
  ...
  app.post('/api/auth/verify', (req, res) => {
    const { phone, code, username, handle, avatar } = req.body || {};
    ...
    // A brand-new number must pick a public username before it gets an account.
    if (!existing) {
      return res.json({ needsProfile: true, phone: target });
    }

    const user = store.upsertUserByPhone(target, { username, handle, avatar });
    if (user.error) return res.status(400).json({ error: user.error });
    store.consumePhoneVerification(target);
    const token = store.createSession(user.id);
    res.json({ token, user: store.publicUser(user, { self: true }) });
  });

  // Completes sign-up for a number that just verified but had no profile yet.
  app.post('/api/auth/register', (req, res) => {
    const { phone, username, handle, avatar } = req.body || {};
    const target = String(phone || '').trim();
    ...
    if (!store.normalizeHandle(handle)) {
  ...
  app.post('/api/auth/session', (req, res) => {
    ...
    res.json({ user: store.publicUser(user, { self: true }) });
  });

  // Look up people by @username — friends do not need your phone number.
  app.get('/api/messenger/users/search', (req, res) => {
    const q = String(req.query.q || '');
    const except = String(req.query.except || '');
    res.json(store.searchUsers(q, except || null));
  });
