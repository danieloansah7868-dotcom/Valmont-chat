...
function issueViewOnceGrant(userId, attachment) {
...
function attach(httpServer, app) {
  ...
  app.post('/api/auth/register', authLimiter, (req, res) => {
    ...
    if (cleanType === 'business' && String(businessProfile?.name || '').trim().length < 2) {
      return res.status(400).json({ error: 'Business name must be at least 2 characters' });
    }
    const user = store.upsertUserByPhone(target, {
      username, handle, avatar, accountType: cleanType,
      businessProfile: cleanType === 'business' ? businessProfile : null,
    });
  ...
  app.post('/api/messenger/messages/:chatId/:messageId/view-once', authRequired, chatLockLimiter, (req, res) => {
    ...
    const token = issueViewOnceGrant(req.auth.user.id, opened.attachment);
    const chat = store.getChat(req.params.chatId);
    if (chat) {
  ...
  app.get('/api/messenger/view-once-media/:grant', authRequired, (req, res) => {
    ...
    if (!grant || grant.userId !== req.auth.user.id || grant.expiresAt <= Date.now()
    ...
    // Consume before touching disk so concurrent/retried requests cannot reuse
    // the same capability, even if delivery later fails.
    viewOnceGrants.delete(digest);
  ...
  io.on('connection', (socket) => {
    ...
    // ── Send message ──────────────────────────────────────────────────
    socket.on('message:send', ({ chatId, text, file, type, replyTo, tempId, clientId, viewOnce }, ack) => {
      const user = me();
      if (!user) return;
