...
const {
  ...
  setVerifyCookie, clearVerifyCookie, verifyContinuationFromRequest,
} = require('./http-auth');
...
function issueViewOnceGrant(userId, attachment, sessionToken, messageId) {
  const token = crypto.randomBytes(32).toString('base64url');
  const now = Date.now();
  for (const [digest, grant] of viewOnceGrants) if (grant.expiresAt <= now) viewOnceGrants.delete(digest);
  viewOnceGrants.set(viewOnceGrantDigest(token), {
    userId,
    sessionDigest: viewOnceGrantDigest(sessionToken),
...
function revokeViewOnceGrantsForSession(sessionToken) {
  const sessionDigest = viewOnceGrantDigest(sessionToken);
  for (const [digest, grant] of viewOnceGrants) {
    if (grant.sessionDigest === sessionDigest) viewOnceGrants.delete(digest);
  }
}

function remainingViewOnceGrants(attachmentId) {
  const now = Date.now();
  let count = 0;
  for (const grant of viewOnceGrants.values()) {
    if (grant.attachmentId === attachmentId && grant.expiresAt > now) count += 1;
  }
  return count;
}
...
function paidBoostsEnabled() {
  return String(process.env.ENABLE_PAID_STORY_BOOSTS || 'true') !== 'false';
}

function accountStorageLimitBytes() {
  return Math.max(1, Number(process.env.MAX_ACCOUNT_STORAGE_MB || 100) || 100) * 1024 * 1024;
}

function pendingUploadLimit() {
  return Math.max(1, Math.floor(Number(process.env.MAX_PENDING_UPLOADS || 20) || 20));
}

function quotaExceeded(userId, extraBytes = 0, extraPending = 0, excludeStorageName = null) {
  const usage = store.protectedMediaUsage(userId, excludeStorageName);
  if (usage.bytes + extraBytes > accountStorageLimitBytes()) {
    return { status: 429, error: 'This account has reached its storage quota' };
  }
  if (extraPending && store.attachmentUsage(userId).pending + extraPending > pendingUploadLimit()) {
    return { status: 429, error: 'Send or discard existing uploads before adding more' };
  }
  return null;
}

function sendProtectedFile(req, res, filePath, mime, extraHeaders = {}) {
  const stat = fs.statSync(filePath);
  const size = stat.size;
  res.setHeader('Accept-Ranges', 'bytes');
  res.setHeader('Content-Type', mime);
  res.setHeader('Cache-Control', extraHeaders['Cache-Control'] || 'private, no-store');
...
function extractMentions(text, chat) {
  const ids = [];
  const handles = [...String(text || '').matchAll(/(?:^|[\s])@([a-z0-9_]{2,20})/gi)].map(match => match[1].toLowerCase());
  if (!chat) return ids;
  if (handles.includes('all') || handles.includes('everyone')) {
    ids.push(...[...chat.members]);
  }
  for (const handle of handles) {
    const mentioned = store.findUserByHandle(handle);
    if (mentioned && chat.members.has(mentioned.id)) ids.push(mentioned.id);
  }
  return [...new Set(ids)];
}
...
function attach(httpServer, app) {
  ...
  let endCallsBlockedByChatLock = () => {};
  const publishedMessageIds = new Set();
  const SOCKET_MESSAGE_RATE_LIMIT = Math.max(1, Math.floor(Number(process.env.SOCKET_MESSAGE_RATE_LIMIT || 120) || 120));
  ...
  const pruneStoryMedia = () => {
    ...
    for (const storageName of store.pruneConsumedViewOnce()) removeProtectedMedia(storageName);
    for (const storageName of store.pruneAbandonedAttachments(Date.now() - 60 * 60 * 1000)) {
      removeProtectedMedia(storageName);
    }
  };
  ...
  app.post('/api/auth/verify', authLimiter, (req, res) => {
    ...
    if (!existing && (!username || String(username).trim().length < 2)) {
      if (check.continuation) setVerifyCookie(res, check.continuation);
  ...
  app.post('/api/auth/register', authLimiter, (req, res) => {
    ...
    const target = String(phone || '').trim();
    const continuation = verifyContinuationFromRequest(req);
    ...
    if (handle !== undefined && handle !== null && handle !== '' && !store.normalizeHandle(handle)) {
      return res.status(400).json({
        error: 'Pick a username: 3–20 characters, start with a letter, letters/numbers/_ only',
      });
    }
    if (accountType !== 'personal' && accountType !== 'business') {
      return res.status(400).json({ error: 'Choose a personal or business account type' });
    }
  ...
  app.post('/api/account/export', authRequired, (req, res) => {
    if (!req.auth.user.pinHash || !store.verifyTwoStepPin(req.auth.user, req.body?.currentPin)) {
      return res.status(401).json({ error: 'Current PIN is incorrect' });
    }
    const exported = store.exportAccountData(req.auth.user.id);
    if (!exported) return res.status(404).json({ error: 'Account not found' });
    res.setHeader('Cache-Control', 'private, no-store');
    res.setHeader('Content-Disposition', `attachment; filename="vchat-account-${req.auth.user.id}.json"`);
    return res.json(exported);
  });

  app.get('/api/account/settings', authRequired, (req, res) => {
    res.setHeader('Cache-Control', 'private, no-store');
    return res.json({ settings: store.getDeviceSettings(req.auth.user.id) });
  });
  app.put('/api/account/settings', authRequired, (req, res) => {
    const settings = store.updateDeviceSettings(req.auth.user.id, req.body || {});
    if (!settings) return res.status(404).json({ error: 'Account not found' });
    try { store.flush(); } catch {
      return res.status(503).json({ error: 'Settings could not be saved' });
    }
    res.setHeader('Cache-Control', 'private, no-store');
  ...
  app.post('/api/account/chat-lock/lock', authRequired, (req, res) => {
    ...
    revokeViewOnceGrantsForSession(req.auth.token);
    endCallsBlockedByChatLock(req.auth.user.id, req.auth.token);
    refreshChatLockSockets(req.auth.user.id);
  ...
  app.get('/api/messenger/users/search', authRequired, (req, res) => {
    res.setHeader('Cache-Control', 'private, no-store');
    return res.json(store.searchUsers(req.query.q, req.auth.user.id));
  });
  ...
  app.post('/api/messenger/upload', authRequired, (req, res, next) => {
    upload.single('file')(req, res, error => {
      ...
      const removeUpload = () => { try { fs.unlinkSync(req.file.path); } catch { /* already gone */ } };
      ...
      if (/^image\//i.test(req.file.mimetype)) {
        let detected;
        try { detected = detectedImageMime(req.file.path); } catch (readError) {
          removeUpload();
          return next(readError);
        }
        if (detected !== String(req.file.mimetype).toLowerCase()) {
          removeUpload();
          return res.status(415).json({ error: 'The file contents do not match the declared image type' });
        }
      } else if (/^video\//i.test(req.file.mimetype)) {
        let detected;
        try { detected = detectedVideoMime(req.file.path, req.file.mimetype); } catch (readError) {
          removeUpload();
          return next(readError);
        }
        if (!detected) {
          removeUpload();
          return res.status(415).json({ error: 'The file contents do not match the declared video type' });
        }
      }
      const blocked = quotaExceeded(req.auth.user.id, req.file.size, 1);
      if (blocked) {
        removeUpload();
        return res.status(blocked.status).json({ error: blocked.error });
      }
  ...
  app.delete('/api/messenger/upload/:attachmentId', authRequired, (req, res) => {
    const discarded = store.discardAttachment(req.params.attachmentId, req.auth.user.id);
    if (!discarded) return res.status(404).json({ error: 'Upload not found' });
    for (const storageName of discarded.storageNames || []) removeProtectedMedia(storageName);
    return res.json({ ok: true });
  });
  ...
  app.post('/api/messenger/messages/:chatId/:messageId/view-once', authRequired, chatLockLimiter, (req, res) => {
    ...
    const token = issueViewOnceGrant(req.auth.user.id, opened.attachment, req.auth.token, opened.message.id);
    const chat = store.getChat(req.params.chatId);
    if (chat) {
  ...
  app.get('/api/messenger/view-once-media/:grant', authRequired, (req, res) => {
    ...
    if (grant.userId !== req.auth.user.id
        || grant.sessionDigest !== viewOnceGrantDigest(req.auth.token)
    ...
    if (!attachment) return res.status(404).json({ error: 'View Once media is unavailable' });
    const filePath = path.join(UPLOAD_DIR, path.basename(attachment.storageName));
    if (!fs.existsSync(filePath)) return res.status(404).json({ error: 'View Once media is unavailable' });
    return sendProtectedFile(req, res, filePath, attachment.mime, {
      'Cache-Control': 'private, no-store, max-age=0',
      'X-View-Once': 'true',
      'Content-Disposition': 'inline',
    }).on('finish', () => {
      if (remainingViewOnceGrants(grant.attachmentId) > 0) return;
      for (const storageName of store.finalizeViewOnceAttachment(grant.chatId, grant.messageId)) {
        removeProtectedMedia(storageName);
      }
    });
  });
  ...
  app.post('/api/reels', authRequired, reelUploadLimiter, (req, res, next) => {
    reelUpload.single('video')(req, res, error => {
      ...
      const blocked = quotaExceeded(req.auth.user.id, req.file.size);
      if (blocked) {
        fs.unlink(req.file.path, () => {});
        return res.status(blocked.status).json({ error: blocked.error });
      }
  ...
  app.get('/api/reels/:reelId/media', authRequired, (req, res) => {
    ...
    return sendProtectedFile(req, res, filePath, reel.mime, {
  ...
  app.get('/api/stories', authRequired, (req, res) => {
    ...
    return res.json({
      ...
      ads: [houseStoryAd(), ...(paidBoostsEnabled() ? store.listEligibleStoryAds(req.auth.user.id) : [])],
      reactions: store.STORY_REACTIONS,
      backgrounds: store.STORY_BACKGROUNDS,
      maxUploadBytes: STORY_MAX_MB * 1024 * 1024,
      paymentConfigured: paidBoostsEnabled() && valmontPayConfigured(),
  ...
  app.post('/api/stories', authRequired, storyUploadLimiter, (req, res, next) => {
    storyUpload.single('media')(req, res, async error => {
      ...
      let payment = null;
      let boostError = null;
      if (String(req.body?.boost) === 'true' && !paidBoostsEnabled()) {
        boostError = 'Paid boosts are not enabled on this deployment';
      } else if (String(req.body?.boost) === 'true') {
  ...
  const rejectDisabledPaidBoosts = (_req, res) => {
    if (paidBoostsEnabled()) return false;
    res.status(503).json({ error: 'Paid boosts are not enabled on this deployment' });
    return true;
  };
  ...
  app.put('/api/story-ads/:campaignId/control', authRequired, (req, res) => {
    if (rejectDisabledPaidBoosts(req, res)) return;
  ...
  app.post('/api/story-ads/valmontpay/webhook', (req, res) => {
    ...
    if (!paidBoostsEnabled()) return res.status(503).json({ error: 'Paid boosts are not enabled on this deployment' });
    const payment = req.body?.data || {};
    const digest = crypto.createHash('sha256').update(req.rawBody).digest('hex');
    try {
      const result = store.processValmontPayWebhook({
        digest,
      ...
      store.flush();
      if (result.outcome === 'confirmed') notifyStories({ type: 'campaign_paid' });
    } catch {
      return res.status(503).json({ error: 'Payment acknowledgement could not be stored' });
    }
  ...
  app.get('/api/story-ads/:campaignId/payment-ledger', authRequired, (req, res) => {
    if (rejectDisabledPaidBoosts(req, res)) return;
    const admin = isStoryAdAdmin(req.auth.user);
    const mine = store.listStoryCampaigns(req.auth.user.id).find(item => item.id === req.params.campaignId);
    const reviewed = admin
      ? store.listStoryCampaignsForReview(req.auth.user.id).find(item => item.id === req.params.campaignId)
      : null;
    const campaign = mine || reviewed;
    if (!campaign) return res.status(404).json({ error: 'Campaign not found' });
    const entries = store.listPaymentLedger(campaign.id, campaign.advertiser.id);
    res.setHeader('Cache-Control', 'private, no-store');
  ...
  app.post('/api/story-ads/:campaignId/refunds', authRequired, (req, res) => {
    if (rejectDisabledPaidBoosts(req, res)) return;
    if (!isStoryAdAdmin(req.auth.user)) return res.status(403).json({ error: 'Advertising administrator access required' });
    const recorded = store.recordCampaignRefund(req.params.campaignId, {
      actorId: req.auth.user.id,
      amountMinor: req.body?.amountMinor,
      providerRefundId: req.body?.providerRefundId,
      reason: req.body?.reason,
    });
    if (!recorded) return res.status(409).json({ error: 'That refund cannot be recorded' });
    try { store.flush(); } catch {
      return res.status(503).json({ error: 'Refund acknowledgement could not be stored' });
    }
    const campaign = store.listStoryCampaignsForReview(req.auth.user.id)
      .find(item => item.id === req.params.campaignId);
    return res.json({ externallyProcessed: true, campaign });
  });
  ...
  app.post('/api/sms/twilio/inbound', (req, res) => {
    ...
    if (configured) {
      const publicRoot = String(process.env.PUBLIC_APP_URL || '').trim();
      const url = publicRoot ? new URL('/api/sms/twilio/inbound', publicRoot).toString() : '';
      if (!url || !sms.twilioSignatureValid(url, req.body || {}, req.get('x-twilio-signature'))) {
        return res.status(401).type('text/plain').send('invalid signature');
      }
    } else if (process.env.NODE_ENV === 'production') {
      return res.status(503).type('text/plain').send('sms unavailable');
    }
    const stored = store.routeInboundSms(from, body);
    if (stored) {
      const chat = store.getChat(stored.chatId);
      for (const memberId of chat?.members || []) {
        emitToUser(memberId, 'message:new', store.outMessage(stored, memberId));
        pushChats(memberId);
      }
    }
  ...
  endCallsBlockedByChatLock = (userId, sessionToken) => {
    for (const call of [...calls.values()]) {
      if (call.from !== userId && call.to !== userId) continue;
      if (socketCanAccessChat({ data: { sessionToken } }, userId, call.chatId)) continue;
      closeCall(call, { outcome: call.answeredAt ? 'ended' : 'missed', reason: 'chat-relocked' });
    }
  };
  ...
  io.on('connection', (socket) => {
    const guardedEvents = new Set([
      ...
      'messages:starred', 'calls:history', 'chat:startSecret', 'chat:acceptSecret', 'chat:declineSecret',
      'chat:setTransport',
      'call:start', 'call:accept', 'call:decline', 'call:cancel', 'call:end',
      'call:signal', 'call:rate', 'call:ratings',
    ]);
    const originalOn = socket.on.bind(socket);
    socket.on = (event, handler) => originalOn(event, (payload, ack) => {
      ...
      if (guardedEvents.has(event)
        ...
        return typeof ack === 'function' ? ack({ error: 'A valid payload object is required' }) : undefined;
      }
      return handler(payload, ack);
    });
    const messageRate = { count: 0, windowStart: Date.now() };
    ...
    // ── Send message ──────────────────────────────────────────────────
    socket.on('message:send', async ({ chatId, text, file, replyTo, tempId, clientId, viewOnce, silent, encryption, transport }, ack) => {
      const user = me();
      if (!user) return;
      const now = Date.now();
      if (now - messageRate.windowStart >= 60_000) {
        messageRate.count = 0;
        messageRate.windowStart = now;
      }
      if (messageRate.count >= SOCKET_MESSAGE_RATE_LIMIT) {
        socket.emit('rate-limit', { event: 'message:send', retryAfter: 60 });
        return ack?.({ error: 'Too many realtime actions. Try again shortly.' });
      }
      ...
      const useSms = transport === 'sms' && chat.type === 'dm';
      if (useSms && file) return ack?.({ error: 'SMS is text-only. Switch to Cloud to send files.' });
      if (chat.type === 'secret') {
        if (chat.secret?.state !== 'ready') return ack?.({ error: 'This secret chat is not ready' });
        if (file) return ack?.({ error: 'Secret chats are end-to-end text only' });
        if (!store.isValidSecretEnvelope(encryption)) return ack?.({ error: 'A valid encrypted envelope is required' });
      }
      const body = chat.type === 'secret' ? '' : String(text || '').trim().slice(0, 10000);
      const safeFile = chat.type === 'secret' ? null : cleanFile(file, user.id, chatId);
      if (chat.type !== 'secret' && !body && !safeFile) return ack?.({ error: 'Message is empty or the attachment is invalid' });
      let reply = null;
      if (replyTo != null) {
        if (chat.type === 'secret') {
          reply = store.getRawMessage(chatId, String(replyTo.id || replyTo)) ? { id: String(replyTo.id || replyTo) } : null;
          if (!reply) return ack?.({ error: 'The message being replied to is unavailable' });
        } else {
          reply = store.replySnapshot(chatId, String(replyTo.id || replyTo), user.id);
          if (!reply) return ack?.({ error: 'The message being replied to is unavailable' });
        }
      }
      const mime = String(safeFile?.mimeType || safeFile?.mime || '');
      const messageType = safeFile
        ? (safeFile.voice ? 'voice'
          : /^image\//i.test(mime) ? 'image'
            : /^video\//i.test(mime) ? 'video'
              : /^audio\//i.test(mime) ? 'audio'
                : 'file')
        : 'text';

      const publishCommitted = (raw, { duplicate = false } = {}) => {
        try {
          store.flush();
        } catch {
          return ack?.({ error: 'Message storage is temporarily unavailable', retryable: true });
        }
        if (!publishedMessageIds.has(raw.id)) {
          publishedMessageIds.add(raw.id);
          for (const memberId of chat.members) {
            emitToUser(memberId, 'message:new', store.outMessage(raw, memberId));
            pushChats(memberId);
          }
        }
        return ack?.({ tempId, duplicate, message: store.outMessage(raw, user.id) });
      };
      ...
      if (already) return publishCommitted(already, { duplicate: true });
      ...
      const raw = store.addMessage({
        ...
        silent: silent === true,
        transport: useSms ? 'sms' : 'cloud',
        mentions: chat.type === 'secret' ? [] : extractMentions(body, chat),
        encryption: chat.type === 'secret' ? {
          v: 1,
          alg: 'A256GCM',
          iv: String(encryption.iv),
          ct: String(encryption.ct),
        } : null,
      });
      if (!raw) return ack?.({ error: 'This secret message could not be stored' });
      if (useSms) {
        const peerId = [...chat.members].find(id => id !== user.id);
        const peer = store.getUser(peerId);
        if (!peer?.phone) return ack?.({ error: 'This contact has no phone number for SMS' });
        const sent = await sms.sendText(peer.phone, `${user.username}: ${body}`);
        if (sent.error) return ack?.({ error: sent.error, retryable: !!sent.configurationRequired });
      }
    ...
    socket.on('message:delete', ({ chatId, messageId, forEveryone }) => {
      ...
      if (!raw) return;
      for (const storageName of raw.storageNames || []) removeProtectedMedia(storageName);
      if (forEveryone) {
        for (const memberId of chat.members) {
          emitToUser(memberId, 'message:updated', store.outMessage(raw, memberId));
          pushChats(memberId);
    ...
    socket.on('message:forward', ({ chatId, messageId, targetChatIds }, ack) => {
      ...
      if (source.advancedPrivacy || source.type === 'secret') return ack?.({ error: 'Forwarding is disabled by advanced chat privacy' });
    ...
    socket.on('chat:startDM', ({ targetUserId }, ack) => {
      ...
      if (targetUserId === user.id) {
        const saved = store.findOrCreateSaved(user.id);
        socket.join(saved.id);
        emitToUser(user.id, 'chat:new', store.chatView(saved, user.id));
        pushChats(user.id);
        return ack?.({ chat: store.chatView(saved, user.id) });
      }
      if (!store.getUser(targetUserId)) return ack?.({ error: 'Contact not found' });
    ...
    socket.on('chat:startSecret', ({ targetUserId, publicKey }, ack) => {
      const user = me();
      if (!user) return ack?.({ error: 'Not signed in' });
      const created = store.createSecretChat(user.id, targetUserId, publicKey);
      if (created?.error) return ack?.({ error: created.error });
      for (const memberId of created.members) {
        const member = store.getUser(memberId);
        if (member) for (const sid of member.socketIds) {
          const memberSocket = io.sockets.sockets.get(sid);
          if (socketCanAccessChat(memberSocket, memberId, created.id)) memberSocket.join(created.id);
        }
        emitToUser(memberId, 'chat:new', store.chatView(created, memberId));
        pushChats(memberId);
      }
      return ack?.({ chat: store.chatView(created, user.id) });
    });

    socket.on('chat:acceptSecret', ({ chatId, publicKey }, ack) => {
      const user = me();
      if (!user) return ack?.({ error: 'Not signed in' });
      const accepted = store.acceptSecretChat(chatId, user.id, publicKey);
      if (accepted?.error) return ack?.({ error: accepted.error });
      socket.join(accepted.id);
      for (const memberId of accepted.members) {
        emitToUser(memberId, 'chat:new', store.chatView(accepted, memberId));
        pushChats(memberId);
      }
      return ack?.({ chat: store.chatView(accepted, user.id) });
    });

    socket.on('chat:declineSecret', ({ chatId }, ack) => {
      const user = me();
      if (!user) return ack?.({ error: 'Not signed in' });
      const chat = store.getChat(chatId);
      const members = chat ? [...chat.members] : [];
      if (!store.declineSecretChat(chatId, user.id)) return ack?.({ error: 'Secret chat not found' });
    ...
    socket.on('chat:setTransport', ({ chatId, transport }, ack) => {
      const user = me();
      if (!user) return ack?.({ error: 'Not signed in' });
      const next = store.setChatTransport(user.id, chatId, transport);
      if (!next) return ack?.({ error: 'SMS is only available in one-to-one chats' });
      pushChats(user.id);
      return ack?.({ transport: next });
    });
    ...
    socket.on('chat:clear', ({ chatId }) => {
      const user = me();
      if (!accessibleChat(user, chatId)) return;
      const cleared = store.clearChat(user.id, chatId);
      for (const storageName of cleared.storageNames || []) removeProtectedMedia(storageName);
    ...
    socket.on('messages:starred', (_payload, ack) => {
      const user = me();
      if (!user) return ack?.({ error: 'Not signed in' });
      ack?.({ items: store.listStarredMessages(user.id, store.isChatLockSessionUnlocked(socket.data.sessionToken)) });
    });

    socket.on('calls:history', (_payload, ack) => {
      const user = me();
      if (!user) return ack?.({ error: 'Not signed in' });
      ack?.({ items: store.listCallHistory(user.id, store.isChatLockSessionUnlocked(socket.data.sessionToken)) });
    });

    socket.on('users:lookup', ({ query }, ack) => {
      const user = me();
      if (!user) return ack?.([]);
      ack?.(store.searchUsers(query, user.id));
    });
