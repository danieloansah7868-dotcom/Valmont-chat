...
async function paystackRequest(endpoint, options = {}) {
  ...
  try {
    ...
    if (!response.ok || result.status !== true) throw new Error(result.message || 'Payment provider request failed');
    return result.data;
  } finally {
    clearTimeout(timer);
  }
...
async function initializeBoostPayment(campaign, callbackUrl) {
  const reference = `vchat-${campaign.id.replace(/-/g, '')}`;
  const data = await paystackRequest('/transaction/initialize', {
    method: 'POST',
    body: JSON.stringify({
      email: campaign.billingEmail,
      amount: Math.round(Number(campaign.budgetGhs) * 100),
      currency: 'GHS',
      reference,
      ...(callbackUrl ? { callback_url: callbackUrl } : {}),
      metadata: JSON.stringify({ campaignId: campaign.id, product: 'vchat_story_boost' }),
    }),
  });
  return {
    reference: data.reference || reference,
    authorizationUrl: safeHttpUrl(data.authorization_url),
  };
}
...
function verifyPaystackWebhook(req) {
  ...
  if (!secret || !/^[a-f0-9]{128}$/i.test(signature) || !Buffer.isBuffer(req.rawBody)) return false;
...
function attach(httpServer, app) {
  ...
  app.get('/api/stories', authRequired, (req, res) => {
    ...
    return res.json({
      ...
      paymentConfigured: Boolean(process.env.PAYSTACK_SECRET_KEY),
      ...
      if (String(req.body?.boost) === 'true') {
        ...
        try {
          ...
          campaign = store.createStoryCampaign(req.auth.user.id, story.id, {
            ...
            paymentProvider: process.env.PAYSTACK_SECRET_KEY ? 'paystack' : null,
          });
          if (!campaign) {
            ...
            try {
              ...
              const callbackRoot = publicRoot ? new URL('/?boost_return=1', publicRoot).toString() : null;
              const initialized = await initializeBoostPayment(campaign, callbackRoot);
              ...
              payment = { authorizationUrl: initialized.authorizationUrl, reference: initialized.reference };
            } catch (paymentError) {
              const reason = paymentError.code === 'configuration_required' ? 'configuration_required' : 'failed';
              campaign = store.setCampaignPaymentInitialization(campaign.id, req.auth.user.id, {
                provider: 'paystack', initialized: false, error: reason,
              });
  ...
  app.get('/api/story-ads/payment/verify', authRequired, async (req, res) => {
    ...
    try {
      const transaction = await paystackRequest(`/transaction/verify/${encodeURIComponent(reference)}`, { method: 'GET' });
  ...
  app.post('/api/story-ads/paystack/webhook', (req, res) => {
    if (!verifyPaystackWebhook(req)) return res.status(401).json({ error: 'Invalid webhook signature' });
