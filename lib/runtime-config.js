...
function productionConfigReport(env = process.env) {
  ...
  if (envString(env, 'PUBLIC_APP_URL') && !publicAppUrl) {
    errors.push('PUBLIC_APP_URL must be a canonical https origin when it is set');
  } else if (!publicAppUrl) {
    warnings.push('PUBLIC_APP_URL is not set; paid story boosts and passkey origin cross-checks are disabled');
  }
  ...
  const passkeyOriginSet = Boolean(envString(env, 'PASSKEY_ORIGIN'));
  const passkeyRpIdSet = Boolean(passkeyRpId);
  if (passkeyOriginSet || passkeyRpIdSet) {
    if (!passkeyOrigin) {
    ...
    if (!passkeyRpIdSet || !/^[a-z0-9.-]+$/.test(passkeyRpId)) {
    ...
    warnings.push('PASSKEY_ORIGIN/PASSKEY_RP_ID are not set; chat-lock passkeys are disabled');
  }
  ...
  const twilioAny = Boolean(twilioSid || twilioToken || twilioFrom);
  if (twilioAny) {
  ...
  } else if (envString(env, 'SMS_DEV_CODE') === 'true') {
    warnings.push('SMS_DEV_CODE is active: verification codes are shown on screen instead of being sent by Twilio');
  } else {
    errors.push('Twilio SMS is not configured; set the TWILIO_* values or SMS_DEV_CODE=true to allow on-screen codes');
  }

  const turnUrls = envString(env, 'TURN_URLS');
  const turnSecret = envString(env, 'TURN_SECRET');
  const turnAny = Boolean(turnUrls || turnSecret);
  if (turnAny) {
    ...
    warnings.push('TURN is not configured; calls rely on public STUN only');
  }
