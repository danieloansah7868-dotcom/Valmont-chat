...
6. Request a code to a real E.164 number and confirm it arrives in Twilio's
   ...
   unless `SMS_DEV_CODE=true` is set explicitly for a small trusted group.
...
or incomplete and `SMS_DEV_CODE` is not enabled. Set `SMS_DEV_CODE=true` to
fall back to on-screen codes, or configure Twilio for real SMS. A 401 on the
ValmontPay webhook means the signature or raw body was altered in transit.
