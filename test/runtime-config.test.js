...
test('production fails closed unless the core safety and storage settings are explicit', () => {
  ...
  assert.ok(report.errors.some(message => message.includes('TRUST_PROXY')));
  assert.ok(report.errors.some(message => message.includes('Twilio SMS is not configured')));
  // Paid/optional integrations degrade to warnings instead of blocking boot.
  assert.ok(report.warnings.some(message => message.includes('public STUN')));
  assert.ok(report.warnings.some(message => message.includes('passkeys are disabled')));
  assert.throws(
...
test('production boots without Twilio or TURN when the dev-code fallback is explicit', () => {
  ...
  assert.ok(report.warnings.some(message => message.includes('SMS_DEV_CODE is active')));
  assert.ok(report.warnings.some(message => message.includes('public STUN')));
  assert.ok(report.warnings.some(message => message.includes('passkeys are disabled')));
});
