...
test('every Socket.IO command rejects malformed payloads without terminating the process', async t => {
  ...
  // The acknowledgement is a durable boundary, not merely confirmation that a
  // delayed timer was scheduled.
...
test('message acknowledgement recovers idempotently after a persistence write failure', async t => {
  ...
  // Let registration's ordinary debounced save finish before making the
  // snapshot directory read-only. The message flush below is then the only
