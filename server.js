...
// Validate before importing the store: importing the transitional adapter creates
// local directories and can load state, which an unsafe production process must
// never do accidentally.
let runtimeConfig;
try {
  ...
  process.stderr.write(`${JSON.stringify({
    ...
    errors: error.report?.errors || ['Runtime configuration is invalid'],
  })}\n`);
  process.exitCode = 1;
  throw error;
}
...
for (const warning of runtimeConfig.warnings) log('warn', 'runtime_configuration_warning', { warning });

const rawTrustProxy = process.env.TRUST_PROXY || (process.env.NODE_ENV === 'production' ? '' : 'loopback');
...
if (runtimeConfig.production) allowedOrigins.add(new URL(process.env.PUBLIC_APP_URL).origin);
...
const apiLimiter = rateLimit({
  ...
  limit: Math.max(10, Number(process.env.API_RATE_MAX || 600)),
...
// The payment provider signs the exact bytes. Capture them in the parser's verify
// hook before decoding JSON; no body transformation may precede this middleware.
...
// Compatibility endpoint: healthz is deliberately readiness, not mere process liveness.
...
const io = messenger.attach(server, app);
...
app.use(express.static(path.join(__dirname, 'public'), {
  ...
  maxAge: 0,
  setHeaders(res) {
    // Asset names are not content-fingerprinted; always revalidate them so an old
    // client cannot pair stale JavaScript with a newly deployed API contract.
...
function shutdown(signal) {
  ...
  io.close(() => {
    server.close(() => {
      ...
      try {
        // Socket disconnection and call teardown may schedule final mutations;
        // commit all of them before reporting a clean shutdown.
        store.flush();
